...
export default (t: Harness) => {
  ...
  t.test("sourcemap/opt-out-with-rollup-sourcemap-enabled", async () => {
    /**
     * This test verifies that dts({ sourcemap: false }) does NOT enable
     * declarationMap capture, even when Rollup's output sourcemap is true.
     *
     * Scenario: User wants JS sourcemaps but NOT .d.ts sourcemaps.
     * They set Rollup output sourcemap: true but dts({ sourcemap: false }).
     *
     * Expected: The plugin should NOT capture TypeScript's declarationMap
     * for .ts inputs, producing a sparse output map (no hydration).
     */

    const dir = path.resolve(process.cwd(), "tests/sourcemap-fixtures/src");
    const inputPath = path.join(dir, "user.ts");

    // Bundle with sourcemap: false in plugin, but sourcemap: true in Rollup output
    const bundle = await rollup({
      input: inputPath,
    ...
    // The map should exist (Rollup creates it) but should be sparse
    // because declarationMap wasn't enabled to provide input maps
    assert.ok(map, "Output should still have a sourcemap from Rollup");

    // Sources will point to the .ts file (Rollup's module ID)
    const hasSource = map.sources.some((s) => s.includes("user.ts"));
    assert.ok(hasSource, `Map should have source, got: ${map.sources.join(", ")}`);

    // Verify hydration did NOT happen: check that "id:" line maps to column 0
    // (sparse mapping) rather than column > 0 (per-identifier precision)
    const decoded = decode(map.mappings);
    ...
    if (idLineIndex >= 0) {
      const idLineMappings = decoded[idLineIndex] || [];
      const mappedSegment = idLineMappings.find((seg) => seg.length >= 4);

      // Without hydration, source column should be 0 (line-start mapping)
      // With hydration, it would be > 0 (per-identifier precision)
      assert.ok(
        !mappedSegment || mappedSegment[3] === 0,
        `With sourcemap: false, mappings should be sparse (column 0), ` +
          `but got column ${mappedSegment?.[3]}. This suggests hydration happened when it shouldn't.`,
      );
    }
  });
