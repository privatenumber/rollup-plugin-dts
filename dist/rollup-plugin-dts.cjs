...
class ModuleDeclarationFixer {
    ...
    formatChunkReference(chunkFileName, jsExt) {
        ...
        const chunkDir = path__namespace.dirname(this.chunkFileName);
        let relativePath = normalizePath(path__namespace.relative(chunkDir, chunkFileName));
...
function preProcess({ sourceFile, isEntry, isJSON }) {
    ...
    for (const node of sourceFile.statements) {
        if (ts.isModuleDeclaration(node) &&
            ...
            /^\.\.?\//.test(node.name.text)) {
            const resolvedPath = path__namespace.resolve(sourceDir, node.name.text);
...
const getChunkImportSpecifier = (fromChunkFileName, toChunkFileName) => {
    const fromDir = path__namespace.posix.dirname(normalizeChunkPath(fromChunkFileName));
    const toRuntimeFileName = getChunkRuntimeFileName(normalizeChunkPath(toChunkFileName));
    let relativePath = path__namespace.posix.relative(fromDir, toRuntimeFileName);
    if (!relativePath.startsWith(".")) {
        relativePath = `./${relativePath}`;
    }
    return relativePath;
            ...
            for (const chunk of Object.values(bundle)) {
                ...
                if (canSimplyReplace && singleInputMap) {
                    ...
                    const remapped = remapping(chunk.map, (file) => {
                        ...
                        const absolutePath = path__namespace.resolve(chunkDir, file);
