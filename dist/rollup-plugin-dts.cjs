...
const transform = () => {
    ...
    return {
        ...
        renderChunk(inputCode, chunk, options) {
            ...
            const relativeModuleDeclarationFixed = new RelativeModuleDeclarationFixer(chunk.fileName, "magicCode" in typesFixed && typesFixed.magicCode ? typesFixed.magicCode : new MagicString(code), !!options.sourcemap, "./" + path__namespace.basename(chunk.fileName, ".d.ts"));
        ...
        async generateBundle(options, bundle) {
            ...
            for (const chunk of Object.values(bundle)) {
                ...
                for (const source of chunk.map.sources) {
                    if (!source)
                        continue;
                    const absoluteSource = path__namespace.resolve(outputDir, source);
                ...
                if (canSimplyReplace && singleInputMap) {
                    ...
                    newSources = singleInputMap.sources.map((source) => path__namespace.isAbsolute(source) ? path__namespace.relative(outputDir, source) : source);
                    ...
                    newSources = remapped.sources
                        ...
                        if (!source)
                            return source;
                        if (path__namespace.isAbsolute(source)) {
                            return path__namespace.relative(outputDir, source);
                        }
                        return source;
                    })
                        .filter((s) => s !== null);
